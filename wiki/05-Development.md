...
│   ├── src/
│   │   ├── components/
│   │   │   └── Navbar.tsx           # Top navigation bar with mode toggle
│   │   ├── hooks/
│   │   │   └── useSocket.ts         # Socket.IO connection + session management
│   │   ├── pages/
│   │   │   ├── MouseMode.tsx        # Mouse mode UI (drag area + buttons)
│   │   │   └── TouchpadMode.tsx     # Touchpad mode UI (1F/2F gestures)
│   │   ├── App.tsx                  # Root component with 3-state UI
...
│   ├── main.py                      # Entry point, route registration, admin auth
│   ├── socket_handler.py            # WebSocket event handlers
│   ├── session_store.py             # SQLite persistence layer
│   ├── mouse_controller.py          # pyautogui wrapper (move, click, scroll)
│   ├── gesture_processor.py         # Swipe/tap/double-tap detection
...
│   └── start.sh                     # Cloudflare Tunnel launcher (Linux/macOS)
│
├── wiki/                            # Documentation
...
SQLite operations with module-level auto-initialization:
